...
def register(request):
	...
	if len(errors):
		...
		new_user = User.objects.create(first_name=request.POST["first"], last_name=request.POST["last"],
		email=request.POST["email"], password=bcrypt.hashpw("request.POST['pw']".encode(), bcrypt.gensalt()))
...
def login(request):
	...
	if login_info == []:
		...
		return redirect("/")
	elif bcrypt.checkpw("request.POST['pw']".encode(), login_info.password.encode()):
